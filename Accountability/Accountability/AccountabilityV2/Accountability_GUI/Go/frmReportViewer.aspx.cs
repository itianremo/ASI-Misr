...
namespace TSN.ERP.WebGUI.Go
{
	...
	public partial class ReportViewer : System.Web.UI.Page
	{
		...
		{
			TextBoxFrom.Attributes.Add("readonly", "readonly");
			TextBoxTo.Attributes.Add("readonly", "readonly");
			reportType = int.Parse(Request["type"]);
//		    IFormatProvider culture = new CultureInfo("en-US",true);
//			string[] vars = Request["vars"].Split(',');
			// added by sm
			 vars = Request["vars"].Split(',');
			if(!IsPostBack)
			{
				if(vars.Length>1 && reportType!=2)
					TextBoxFrom.Text=DateTime.Parse(vars[1],culture).ToShortDateString();
			}
			//end
			//bool showOffDays;
			//int viewMode;
			//string toolValue =  ConfigurationSettings.AppSettings[ "reportingTool" ];
			 toolValue =  ConfigurationSettings.AppSettings[ "reportingTool" ];
...
//				Panel1.Visible=true;
//				Table2.Visible=false;
//				showOffDays= Boolean.Parse(vars[5]);
//				viewMode = int.Parse(vars[6]);
//				int RowsCount = 0;
...
//	
//	
//					Navigation.BaseObject.SafeMerge(dsAccountabilitySheet,((Navigation.BaseObject) Session[ "navigation" ]).AccountabilityWSObject.GetAccSheet(int.Parse(vars[0]),DateTime.Parse(TextBoxFrom.Text.Trim(),culture),10,showOffDays));
//					//Navigation.BaseObject.SafeMerge(dsAccountabilitySheet,((Navigation.BaseObject) Session[ "navigation" ]).AccountabilityWSObject.GetAccSheet(int.Parse(vars[0]),DateTime.Parse(vars[1],culture),10,showOffDays));
...
//				else
//				{
//					Navigation.BaseObject.SafeMerge(dsAccountabilitySheet,((Navigation.BaseObject) Session[ "navigation" ]).AccountabilityWSObject.GetAccSheet(int.Parse(vars[0]),DateTime.Parse(TextBoxFrom.Text.Trim(),culture),viewMode,showOffDays));
//				   //Navigation.BaseObject.SafeMerge(dsAccountabilitySheet,((Navigation.BaseObject) Session[ "navigation" ]).AccountabilityWSObject.GetAccSheet(int.Parse(vars[0]),DateTime.Parse(vars[1],culture),viewMode,showOffDays));
//				}
//			
//				if ( toolValue == "1" )
//				{
...
////					RepDoc.Load(Server.MapPath("rep/rptEmpAccSheet.rpt"));
////					RepDoc.SetDataSource(dsAccountabilitySheet);
////					for(int i=0;i<RepDoc.DataDefinition.FormulaFields.Count;  i++)
////					{
////						if(RepDoc.DataDefinition.FormulaFields[i].FormulaName=="{@empname}" )
////							RepDoc.DataDefinition.FormulaFields[i].Text = "\"" + vars[2] +"\"";
////						if(RepDoc.DataDefinition.FormulaFields[i].FormulaName=="{@dept}" )
////							RepDoc.DataDefinition.FormulaFields[i].Text = "\"" + vars[3] +"\"";
////						if(RepDoc.DataDefinition.FormulaFields[i].FormulaName=="{@job}" )
////							RepDoc.DataDefinition.FormulaFields[i].Text = "\"" + vars[4] +"\"";
////						if(RepDoc.DataDefinition.FormulaFields[i].FormulaName=="{@rootPath}" )
////							RepDoc.DataDefinition.FormulaFields[i].Text = Server.MapPath("images");
////					}
////					RepDoc.Refresh();
...
//
//							//Add Daysoff table
//							DataTable Daysoff = new DataTable();
//							Daysoff.TableName="Daysoff";
//							Daysoff.Columns.Add(new DataColumn("SunDayoff", typeof(bool)));
//							Daysoff.Columns.Add(new DataColumn("MonDayoff", typeof(bool)));
//							Daysoff.Columns.Add(new DataColumn("TueDayoff", typeof(bool)));
//							Daysoff.Columns.Add(new DataColumn("WedDayoff", typeof(bool)));
//							Daysoff.Columns.Add(new DataColumn("ThurDayoff", typeof(bool)));
//							Daysoff.Columns.Add(new DataColumn("FriDayoff", typeof(bool)));
//							Daysoff.Columns.Add(new DataColumn("SatDayoff", typeof(bool)));
//							DataRow drDaysoff = Daysoff.NewRow();
//							for(int i=0; i<7; i++)
//							{
//								drDaysoff[i] = dsAccountabilitySheet.Tables[3].Rows[i]["HasDaysOff"];
//							}
//							Daysoff.Rows.Add(drDaysoff);
//							dsAccountabilitySheet.Tables.Add(Daysoff);
//							///////////////////
//							///
...
//					string filter;
//					CrystalDecisions.CrystalReports.Engine.ReportDocument RepDoc = new CrystalDecisions.CrystalReports.Engine.ReportDocument();
//					RepDoc.Load(Server.MapPath("rep/rptTotalAccSheet.rpt"));
//					filter = vars.Length >1 ? vars[1] : "";
//					RepDoc.SetDataSource(((Navigation.BaseObject) Session[ "navigation" ]).AccountabilityWSObject.GetTotalAccSheet(DateTime.Parse(vars[0],culture),filter));	
//					RepDoc.Refresh();
//					this.CrystalReportViewer1.ReportSource = RepDoc;
				...
				else if ( toolValue == "2" )
				{
					...
					if (  dsAccountabilitySheet != null )
					{
						...
						streprt1.RegData ( ((Navigation.BaseObject) Session[ "navigation" ]).AccountabilityWSObject.GetTotalAccSheet(DateTime.Parse(vars[0],culture),filter) ); 
						StiWebViewer1.Report = streprt1;
						StiWebViewer1.Visible = true;				
...
//			////---------------This condition has been commented to satisfy Rod--------------------
...
//			////------------------------------------------------------------------------------------
		...
		public TSN.ERP.SharedComponents.Data.dsAccountabilitySheet DecryptWeeklyNotes(TSN.ERP.SharedComponents.Data.dsAccountabilitySheet ds)
		{
			...
			/*foreach(DataRow dr in ds.Tables[0].Rows)
			{
				dr["RecoredType"]= dr["RecoredType"].ToString();
				dr["taskname"]= dr["taskname"].ToString();
		...
		protected void ButtonTskSumm_Click(object sender, ImageClickEventArgs e)
		{
			if (reportType == 1)
			{
				mWeeklySummaryReport();
			}
			else
			GetTaskSummary( );	
		}
		private void mWeeklySummaryReport()
		{
			#region  Accountability sheet for only one given employee
