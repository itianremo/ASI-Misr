function changeimg(s)
{
      ...
      var openedpic='contents.gif';
       var closedpic='contents2.gif';
       var Elements = document.getElementsByTagName("IMG");
        
        for(i=0; i<Elements.length; i++)
        {
			if(Elements[i].id != s.id)
			{
				if(Elements[i].src.substr(40,13)=='contents.gif')
				{
					Elements[i].src=closedpic;
				}
			}			
         }
         s.src=openedpic;
...
function getDescription( id )
		{
			//alert(id);
		    document.getElementById( "LabelTitle" ).innerText = document.getElementById(id ).innerText;
			HelpDetails.GetNodeDescription( id,getDescription_CallBack);
		
		}	
		function getDescription_CallBack(response)
		{
			var desc = response.value;
			var arr = desc.split( "$" );
			document.getElementById('imgDiv').innerHTML = "";
			
			if (arr[0] != ""  )
			{
				document.getElementById('div1').innerText = arr[0];			
			}
			
			if (arr[1] != "" )
			{
			 
				var imgTag =  "<IMG src='XMLHelpFiles/Images/" + arr[1] + "'>";
	...
	if (nextDIV.style.display == 'none') {
		...
		if (node.childNodes.length > 0) {
			...
			if (node.childNodes.item(0).nodeName == "IMG") {
				...
				node.childNodes.item(0).src = getImgDirectory(node.childNodes.item(0).src) + "contents.gif";
	...
	// Collapse the branch if it IS visible
		...
		if (node.childNodes.length > 0) {
			...
			if (node.childNodes.item(0).nodeName == "IMG") {
  				...
  				node.childNodes.item(0).src = getImgDirectory(node.childNodes.item(0).src) + "contents2.gif";
